...
class _TelegramLoginPageState extends State<TelegramLoginPage> {
    ...
    try {
      ...
      if (data['token'] != null) {
        ...
        if (mounted) {
          ...
          KazumiDialog.showToast(message: 'Telegram 登录成功 🎉');
  ...
  Future<void> _bindToken(String appToken) async {
    ...
    try {
      ...
      request.add(utf8.encode(jsonEncode({'provider': 'telegram', 'app_token': appToken})));
      final response = await request.close();
      final body = await response.transform(utf8.decoder).join();
      client.close();
      final data = jsonDecode(body) as Map<String, dynamic>;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(widget.bindMode ? '绑定 Telegram' : 'Telegram 登录')),
      body: ListView(padding: const EdgeInsets.all(24), children: [
        const SizedBox(height: 40),
        Container(width: 80, height: 80,
          decoration: BoxDecoration(color: const Color(0xFF0088CC).withAlpha(25), shape: BoxShape.circle),
          child: Image.asset('assets/images/icons/telegram.png', width: 40, height: 40)),
