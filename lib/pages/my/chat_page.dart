...
class _ChatPageState extends State<ChatPage> {
  ...
  Widget _buildSyncInvite(SocialChatMessage m) {
    ...
    return Text('🎬 邀请一起看', style: TextStyle(fontSize: 13, color: color));
  }
  ...
  Future<void> _acceptSyncInvite(Map<String, dynamic> data) async {
    ...
    // 打开番剧详情页，用户选源播放后自动加入房间
    try {
      KazumiDialog.showLoading(msg: '加载中');
      final item = await BangumiApi.getBangumiInfoByID(id);
      KazumiDialog.dismiss();
      if (item == null || !mounted) {
        KazumiDialog.showToast(message: '番剧加载失败');
        return;
      }
      ...
      KazumiDialog.dismiss();
      KazumiDialog.showToast(message: '加载失败');
    }
  }

  /// 生成 Syncplay 用户名（yhdm + 4 位小写字母）
  ...
  Future<void> _openAnimeDetail(int id) async {
    try {
      ...
      KazumiDialog.dismiss();
      KazumiDialog.showToast(message: '打开详情失败');
    }
  }
}
