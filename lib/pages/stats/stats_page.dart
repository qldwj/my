...
class _StatsPageState extends State<StatsPage> {
  ...
  Future<void> _captureShareImage() async {
    try {
      ...
      final fileName =
          'kazumi_weekly_report_${DateTime.now().millisecondsSinceEpoch}.png';
      ...
      if (isDesktop()) {
        ...
        if (mounted) {
          KazumiDialog.showToast(message: '已保存到 ${file.path}');
        }
      } else {
        ...
        KazumiDialog.showToast(
          message: result.isSuccess ? '已保存到相册 📸' : '保存失败：${result.errorMessage}',
        );
      }
    } catch (e) {
