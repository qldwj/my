...
abstract class _InfoController with Store {
  ...
  Future<void> _mergeCustomComments(int subjectId) async {
    try {
      final custom = await CustomCommentApi.fetch(subjectId: subjectId);
      if (custom.isEmpty) return;
      final items = custom.map((c) => CommentItem(
            user: User(
              id: -c.id,
              username: 'server',
              nickname: c.sender.isEmpty ? '樱花用户' : c.sender,
              avatar: UserAvatar(
                // 樱花头像走镜像（wsrv.nl 中转），原服务器不稳定时也能加载
                small:
                    '${ApiEndpoints.bangumiImageProxyBase}${Uri.encodeComponent('https://qlyyz.xyz/logo.webp')}',
                medium:
                    '${ApiEndpoints.bangumiImageProxyBase}${Uri.encodeComponent('https://qlyyz.xyz/logo.webp')}',
                large:
                    '${ApiEndpoints.bangumiImageProxyBase}${Uri.encodeComponent('https://qlyyz.xyz/logo.webp')}',
  ...
  /// ⭐ 发表评论到樱花服务器，成功后刷新评论列表
  Future<String?> addCustomComment(String text) async {
