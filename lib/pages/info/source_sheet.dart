...
class ChapterErrorException implements Exception {
  ...
  ChapterErrorException(this.pluginName);
}

const double materialBottomSheetRadius = 16.0;
...
class _SourceSheetState extends State<SourceSheet>
  ...
  List<Plugin> get _filteredPlugins {
    ...
    if (_filterText.isNotEmpty) {
      return sorted.where((p) => p.name.toLowerCase().contains(_filterText)).toList();
    }
    return sorted;
  }
  ...
  void initState() {
    ...
    _sourceTabController.addListener(() {
      if (_sourceTabController.indexIsChanging ||
          _sourceTabController.index != _currentTabIndex) {
        ...
        child: Padding(
          ...
          child: SizedBox(
            ...
            child: Column(
              ...
              children: [
                Text('自动验证中', style: Theme.of(context).textTheme.titleLarge),
  ...
  Widget buildPluginView(Plugin plugin, List<Widget> cardList) {
    ...
    if (status == PluginSearchStatus.noResult) {
      return GeneralErrorWidget(
        ...
        actions: [
          GeneralErrorButton(onPressed: () => showAliasSearchDialog(plugin.name), text: '别名检索'),
          GeneralErrorButton(onPressed: () => showCustomSearchDialog(plugin.name), text: '手动检索'),
        ],
      );
    }
    ...
    return Column(
      children: [
        Expanded(child: ListView(children: cardList)),
      ...
      builder: (context) {
        return AlertDialog(
          ...
          actions: [
            ...
            TextButton(
              onPressed: () => submit(customKeyword),
              child: const Text('确认'),
            ),
          ],
        );
          ...
          Expanded(
            child: Observer(
              builder: (context) => TabBarView(
                controller: _sourceTabController,
                children: [
                  // ⭐ 关键修复：使用 `}),` 正确闭合 List.generate
                  ...List.generate(
                    _filteredPlugins.length,
                    (pluginIndex) {
                          ...
                          for (var searchItem in searchResponse.data) {
                            cardList.add(
                              Card(
                                ...
                                child: InkWell(
                                  borderRadius: BorderRadius.circular(materialBottomSheetRadius),
                                  onTap: () => _playWithSource(plugin, searchItem),
                                  onLongPress: () => _addResultToQueue(plugin, searchItem),
                                  child: Padding(
...
class _CaptchaDialogState extends State<_CaptchaDialog> {
  final ValueNotifier<String?> _captchaImageNotifier = ValueNotifier<String?>(null);
      ...
      child: Padding(
        ...
        child: SizedBox(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.center,
            children: [
              Text('验证码验证', style: Theme.of(context).textTheme.titleLarge),
              const SizedBox(height: 4),
              Text('${widget.pluginName} 需要验证码验证', style: Theme.of(context).textTheme.bodySmall),
              const SizedBox(height: 20),
              ...
              const SizedBox(height: 20),
              ListenableBuilder(
                listenable: Listenable.merge([_captchaImageNotifier, _submittingNotifier]),
                builder: (context, _) {
                            ...
                            actions: [
                              ...
                              TextButton(
                                onPressed: () {
                                  ...
                                  aliasNotifier.value = List.from(widget.aliases);
