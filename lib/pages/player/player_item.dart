...
class PlayerItem extends StatefulWidget {
  const PlayerItem({
    ...
    this.onAddToPlayQueue,
    this.onOpenPlayQueue,
    this.onPlayNextFromQueue,
    this.onInviteSyncPlay,
  });
  ...
  /// 🆕 连播队列回调（由 VideoPage 提供）
  final VoidCallback? onAddToPlayQueue;
  final VoidCallback? onOpenPlayQueue;

  /// 最后一集播完时尝试从队列取下一项播放（同步判断并触发异步播放），
  /// 返回是否已接管播放（true 则不再标记"看过"）
  final bool Function()? onPlayNextFromQueue;

  /// 🆕 邀请好友一起看（Syncplay）
  final VoidCallback? onInviteSyncPlay;
...
class _PlayerItemState extends State<PlayerItem>
  ...
  /// 🆕 倍速记忆：是否已应用本番记忆倍速（避免每 tick 重复设置）
  bool _appliedPlaybackSpeedMemory = false;
  ...
  Future<void> skipOP() async {
    ...
    TimeSavedService.add(playerController.playback.buttonSkipTime);
  }
  ...
  Future<void> setPlaybackSpeed(double speed) async {
    ...
    // 🆕 倍速记忆：手动改倍速时按番剧记录
    if (GStorage.getSetting(SettingsKeys.playSpeedMemoryEnabled)) {
      unawaited(PlaySpeedMemoryService.setSpeed(
          videoPageController.bangumiItem.id, speed));
    }
  }
  ...
  Timer getPlayerTimer() {
    return Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      if (!_appliedPlaybackSpeedMemory &&
        ...
        _appliedPlaybackSpeedMemory = true;
        final savedSpeed =
            PlaySpeedMemoryService.getSpeed(videoPageController.bangumiItem.id);
        if (savedSpeed != null && savedSpeed > 0) {
          unawaited(playerController.setPlaybackSpeed(savedSpeed));
        }
      }
      if (playerController.playback.playerPlaying &&
          !videoPageController.loading &&
      ...
      if (playerController.playback.completed && !videoPageController.loading) {
        // ⚡ 省时统计：每集结束提示（本集跳过了多少秒）
        if (TimeSavedService.sessionSeconds > 0) {
          final saved = TimeSavedService.sessionSeconds;
          final totalMin = TimeSavedService.totalSeconds ~/ 60;
          final totalSec = TimeSavedService.totalSeconds % 60;
          KazumiDialog.showToast(
            message: '⚡ 本集为你省了 $saved 秒（累计已省 $totalMin 分 $totalSec 秒）',
            duration: const Duration(seconds: 3),
          );
          TimeSavedService.resetSession();
        }
        if (playerController.playback.resumedNearEnd) {
          ...
          // ⭐ 最后一集看完 → 先尝试从连播队列接播下一部番
          final queueHandled = widget.onPlayNextFromQueue != null
              ? widget.onPlayNextFromQueue!()
              : false;
          if (!queueHandled) {
            // 队列为空 → 自动把收藏状态从"在看"切到"看过"
            _autoMarkWatched();
          }
        }
      }
  ...
  void _scheduleAndroidPIPSourceRectSync() {
    if (_pipSourceRectSyncScheduled) return;
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final sourceRect = offset & box.size;
      if (sourceRect != _lastPipSourceRect) {
                  ...
                  child: Stack(alignment: Alignment.center, children: [
                    ...
                    (needFullPanel(context))
                        ? PlayerItemPanel(
                            ...
                            skipOP: skipOP,
                            onAddToPlayQueue: widget.onAddToPlayQueue,
                            onOpenPlayQueue: widget.onOpenPlayQueue,
                            onInviteSyncPlay: widget.onInviteSyncPlay,
                          )
