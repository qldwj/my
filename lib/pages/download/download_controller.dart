...
abstract class _DownloadController with Store {
  ...
  Future<void> init() async {
    ...
    // 🆕 下载完成通知
    _downloadManager.onEpisodeCompleted = _onEpisodeCompleted;
  ...
  /// 🆕 单集下载完成 → 系统通知
  void _onEpisodeCompleted(String recordKey, int episodeNumber,
      DownloadEpisode episode) {
    try {
      if (!GStorage.getSetting(SettingsKeys.downloadCompleteNotify)) return;
      final record = _repository.getRecord(recordKey);
      final bangumiName = record?.bangumiName ?? '';
      final epName = episode.episodeName.isNotEmpty
          ? episode.episodeName
          : '第$episodeNumber集';
      unawaited(AnimeUpdateNotificationService.showNotification(
        title: '📥 下载完成',
        body: bangumiName.isNotEmpty
            ? '《$bangumiName》$epName 已下载完成'
            : '$epName 已下载完成',
        payload: 'download',
      ));
    } catch (e) {
      KazumiLogger()
          .w('DownloadController: episode completed notify failed', error: e);
    }
  }
