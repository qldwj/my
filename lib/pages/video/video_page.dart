...
class _VideoPageState extends State<VideoPage>
  ...
  /// 🆕 生成 Syncplay 用户名（yhdm + 4 位小写字母，共 8 位纯字母）
  String _genSyncUsername() {
    const chars = 'abcdefghijklmnopqrstuvwxyz';
    final r = Random();
    final buf = StringBuffer('yhdm');
    for (var i = 0; i < 4; i++) {
  ...
  /// 🆕 邀请好友"一起看"：创建 Syncplay 房间 + 发邀请消息
  Future<void> _inviteSyncPlay() async {
    try {
      // 1. 生成房间号（6 位数字）+ 用户名
      final room = (Random().nextInt(900000) + 100000).toString();
      final username = _genSyncUsername();
