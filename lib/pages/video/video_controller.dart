...
abstract class _VideoPageController with Store implements Disposable {
  ...
  void _triggerPreloadForEpisode(int episode, int road) {
    ...
    // ⭐ 检查是否正在重试中
    final retryCount = _preloadRetryCount[resolved.listIndex] ?? 0;
    if (retryCount >= 3) {
      KazumiLogger().w('⚠️ 预加载重试次数已达上限: 第${resolved.listIndex}集');
      return;
    }
  ...
  /// ⭐ 新增：带重试机制的预加载
  Future<void> _preloadEpisodeWithRetry(
    String urlItem,
    EpisodeRef resolved, {
    int maxRetries = 2,
    ...
    int attempt = 0;
    bool success = false;
    final episodeKey = resolved.listIndex;
    
    while (attempt <= maxRetries && !success) {
      attempt++;
      try {
        KazumiLogger().i('🔄 预加载尝试 $attempt/${maxRetries + 1}: 第$episodeKey集');
        
        // ⭐ 每次都重新创建 service，避免复用导致的问题
        ...
        final source = await _videoSourceService!.resolve(
          ...
          onTimeout: () {
            throw VideoSourceTimeoutException('预加载超时 (第$attempt次尝试)');
          },
        );
        
        if (source.url.isNotEmpty) {
          _preloadedVideoUrls[episodeKey] = source.url;
          _preloadRetryCount.remove(episodeKey); // ⭐ 成功后清除重试计数
          KazumiLogger().i('✅ 预加载完成: 第$episodeKey集 (尝试 $attempt)');
          success = true;
        } else {
          throw Exception('预加载返回空URL');
        }
      } catch (e) {
        KazumiLogger().w('⚠️ 预加载失败 (尝试 $attempt): 第$episodeKey集', error: e);
        _preloadRetryCount[episodeKey] = attempt;
    ...
    // ⭐ 所有尝试都失败后，标记预加载完成（即使失败也要释放锁）
    _preloadInFlight = false;
    
    // ⭐ 如果预加载彻底失败，记录日志但不阻塞用户
    if (!success) {
      KazumiLogger().w('❌ 预加载彻底失败: 第$episodeKey集，已尝试 ${maxRetries + 1} 次');
      // ⭐ 不清除重试计数，让后续可以再次尝试
    }
  }
  ...
  Future<void> preloadNextEpisode() async {
    ...
    try {
      ...
      final source = await _videoSourceService!.resolve(
        ...
        const Duration(seconds: 25),
        onTimeout: () {
          throw VideoSourceTimeoutException('预加载超时');
        },
      );
      
      if (source.url.isNotEmpty) {
        _preloadedVideoUrls[resolved.listIndex] = source.url;
        _preloadRetryCount.remove(resolved.listIndex);
        KazumiLogger().i('VideoPageController: 预加载下一集 ${resolved.listIndex} 完成');
      }
    } catch (e) {
      _preloadRetryCount[resolved.listIndex] = (retryCount + 1);
  ...
  Future<void> _playWithPreloadedUrl(
    ...
    if (session.isActive && initialized) {
      ...
      // ⭐ 播放成功后，预加载下一集
      unawaited(_triggerPreloadForEpisode(
        resolvedEpisode.listIndex + 1, 
        resolvedEpisode.roadIndex
      ));
  ...
  Future<void> changeEpisode(
    ...
    if (preloadedUrl != null && preloadedUrl.isNotEmpty) {
      ...
      // ⭐ 播放当前集完成后，预加载下一集（在 _playWithPreloadedUrl 中已经触发）
      return;
    }
