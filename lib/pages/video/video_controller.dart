...
abstract class _VideoPageController with Store implements Disposable {
  ...
  /// ⭐ 主动预加载指定集数（返回 Future，可 await）
  Future<void> _triggerPreloadForEpisode(int episode, int road) async {
    ...
    await _preloadEpisodeWithRetry(
      urlItem,
      resolved,
      maxRetries: 2,
    ...
    while (attempt <= maxRetries && !success) {
      ...
      try {
        ...
        final source = await _videoSourceService!.resolve(
          ...
          onTimeout: () {
            // ⭐ 修复：传递 Duration 类型
            throw VideoSourceTimeoutException(const Duration(seconds: 25));
          },
        );
        
        if (source.url.isNotEmpty) {
          _preloadedVideoUrls[episodeKey] = source.url;
          _preloadRetryCount.remove(episodeKey);
  ...
  Future<void> _playWithPreloadedUrl(
    ...
    if (session.isActive && initialized) {
      ...
      // ⭐ 修复：不再使用 unawaited，直接调用（现在是 async 方法）
      _triggerPreloadForEpisode(
