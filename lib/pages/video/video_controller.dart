...
abstract class _VideoPageController with Store implements Disposable {
  ...
  Future<void> _playWithPreloadedUrl(
    ...
    _finishLoading();
    KazumiLogger().i(
        'VideoPageController: using preloaded URL for ${resolvedEpisode.listIndex}');
  ...
  Future<void> _loadCustomDanmakus(
    ...
    try {
      ...
      if (items.isEmpty) {
        // 🔧 调试提示：没有拉到自建弹幕
        KazumiDialog.showToast(message: '自建弹幕 0 条（bangumiId=${params.bangumiId} ep=${params.episode}）');
        return;
      }
      ...
      KazumiDialog.showToast(
          message: '✅ 自建弹幕 ${items.length} 条已加载');
      KazumiLogger()
