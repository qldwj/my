...
class AuthService {
  ...
  /// 🆕 生成 OAuth 授权地址（GitHub / QQ）—— 聚合登录平台
  /// [bindToken] 传入当前登录 token 时是"绑定"流程，否则是"登录"流程
  static String oauthAuthorizeUrl(String provider, {String? bindToken}) {
    // 聚合登录授权页（appid/appkey 由平台提供）
    const base =
        'https://open.juhedenglu.cn/connect.php?act=login&appid=158502&appkey=ab7f0a7757fa37453395fd54513af6bc';
    final type = provider == 'github' ? 'github' : 'qq';
    final redirect = Uri.encodeComponent(
        'https://qlyyz.xyz/api/qqgithub.php');
    final state = bindToken ?? 'login';
    return '$base&type=$type&redirect_uri=$redirect&state=$state';
  }
