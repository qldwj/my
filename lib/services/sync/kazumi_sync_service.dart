...
/// - [uploadAll]：上传全部本地数据覆盖云端（本地优先）
/// - [downloadAll]：下载全部云端数据覆盖本地（换设备使用）
...
class KazumiSyncService {
  ...
  /// 单次上传历史条数上限（控制请求体积；用户要求只保留最近 5 条）
  static const int _historyUploadLimit = 5;

  // ========================== 🆕 全量上传/下载（URL 已写死） ==========================

  /// 上传本地全部数据到云端（覆盖云端数据）
  /// 返回结果消息列表，与 syncAll 格式一致
  static Future<List<String>> uploadAll() async {
    ...
    try {
      // 1. 收集本地收藏
      final collectList = GStorage.collectibles.values.map((c) {
        return {
          'id': c.bangumiItem.id,
      ...
      // 3. 收集设置
      final weeklyGoal =
          GStorage.getSetting<int>(SettingsKeys.weeklyWatchGoal);
      final settings = {
        'weeklyWatchGoal': weeklyGoal,
      };
      ...
      if (result['success'] != true) {
        return ['❌ 上传失败: ${result['error'] ?? '未知错误'}'];
      }

      return ['✅ 上传成功，版本 ${result['version'] ?? '?'}'];
    } catch (e, st) {
      KazumiLogger().e('上传全部数据失败', error: e, stackTrace: st);
      return ['❌ 上传失败: $e'];
    }
  }
  ...
  /// 从云端下载全部数据覆盖本地（云端优先）
  /// 返回结果消息列表
  static Future<List<String>> downloadAll() async {
    final token = AuthService.getLocalToken();
    if (token == null || token.isEmpty) {
      return ['❌ 未登录'];
    }
    ...
    try {
      ...
      final result = json.decode(response.body);
      if (result['success'] != true) {
        return ['❌ 下载失败: ${result['error'] ?? '未知错误'}'];
      }

      // 2. 解析数据
      final collectList = result['collect'] as List? ?? [];
      ...
      // 5. 应用设置
      final weeklyGoal = settings['weeklyWatchGoal'];
      if (weeklyGoal is int && weeklyGoal > 0) {
        await GStorage.putSetting(SettingsKeys.weeklyWatchGoal, weeklyGoal);
      }

      return [
        '✅ 下载完成：收藏 $collectCount 项，历史 $historyCount 项',
        '✅ 设置已应用'
      ];
    } catch (e, st) {
      KazumiLogger().e('下载全部数据失败', error: e, stackTrace: st);
      return ['❌ 下载失败: $e'];
    }
  }
  ...
  /// 通用设置同步（本周目标等）：上传本地 → 拉取云端合并
  static Future<String> syncSettings() async {
    final token = AuthService.getLocalToken();
    if (token == null || token.isEmpty) {
      return '❌ 未登录';
    }
    try {
      ...
      if (upRes['error'] != null) {
        return '❌ 上传设置失败: ${upRes['error']}';
      }
      // 2. 拉取云端设置并应用（本地为 0 时用云端值）
      final remoteRes = await AuthService.getRemoteSync();
      if (remoteRes['error'] != null) {
        return '❌ 拉取云端失败: ${remoteRes['error']}';
      }
      ...
      if (settings is Map) {
        ...
        if (remoteGoal is int && remoteGoal > 0) {
          if (GStorage.getSetting<int>(SettingsKeys.weeklyWatchGoal) <= 0) {
            await GStorage.putSetting(
                SettingsKeys.weeklyWatchGoal, remoteGoal);
          }
        }
      }
      return '✅ 设置同步完成';
    } catch (e, st) {
      KazumiLogger().e('设置云同步失败', error: e, stackTrace: st);
      return '❌ 设置同步失败: $e';
    }
  }
  ...
  static Future<List<String>> syncAll() async {
    ...
    results.add(await syncSettings());
    return results;
  }
