...
/// 🆕 单集下载完成回调（用于"下载完成通知"）
typedef EpisodeCompletedCallback = void Function(
...
abstract class IDownloadManager {
  ...
  EpisodeCompletedCallback? onEpisodeCompleted;
...
class DownloadManager implements IDownloadManager {
  ...
  Future<void> _runEpisodeDownload({
    ...
    try {
      ...
      _notifyProgress(task.recordKey, task.episodeNumber, episode);
      // 🆕 单集完成回调（下载完成通知）
      onEpisodeCompleted?.call(task.recordKey, task.episodeNumber, episode);
