...
class CssRuleStrategy {
  ...
  /// 🆕 递归解析视频直链（支持多级嵌套 URL）
  ///
  /// 与 [extractVideoUrl] 的区别：当 matchNestedUrl 命中且结果不是
  /// 视频直链时，会通过 [fetcher] 抓取嵌套页再次提取，最多递归 3 层。
  /// [fetcher] 由调用方提供（负责带 cookie/referer 的页面抓取）。
  ///
  /// 返回空串表示无结果；返回视频直链（mp4/m3u8/flv/webm/mkv 等）；
  /// 无法继续递归时返回当前已匹配的 URL（交由上层 WebView 回退兜底）。
  Future<String> resolveVideoUrl(
    ...
    for (var depth = 0; depth < 3; depth++) {
      final url = extractVideoUrl(html, rule);
      if (url.isEmpty) return '';
      if (_looksLikeVideoUrl(url)) return url;
      // 不是直链 → 视为嵌套页地址，抓取后继续提取
      if (fetcher == null) return url;
      if (visited.contains(url)) return url; // 防循环
      visited.add(url);
      try {
    ...
    return extractVideoUrl(html, rule);
  }
  ...
  bool _looksLikeVideoUrl(String url) {
    ...
    // 页面脚本扩展名 → 一定不是视频直链，避免误判
    const pageExtensions = [
      '.html', '.htm', '.php', '.asp', '.aspx', '.jsp',
      '.shtml', '.xhtml', '.cgi', '.js', '.css', '.json', '.xml',
    ];
    if (pageExtensions.any(lower.contains)) return false;
    // 视频扩展名（含带参数情况如 .mp4?token=xxx）
    const videoExtensions = [
      '.mp4', '.m3u8', '.flv', '.webm', '.mkv', '.ts',
      '.mpd', '.m4s', '.m4v', '.mov', '.avi', '.wmv',
    ];
    if (videoExtensions.any(lower.contains)) return true;
    // 常见 CDN/播放器直链路径特征（无页面扩展名时）
    const videoHints = [
      '/hls/', '/hls?', '/m3u8', '/video/', '/videoplay',
      '/play/', '/player/', '/live/', '/stream', '/media/',
      '/vod/', '/playlist.m3u8',
    ];
    if (videoHints.any(lower.contains)) return true;
    return false;
  }
