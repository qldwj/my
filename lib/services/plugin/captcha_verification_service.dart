...
class CaptchaVerificationService {
  ...
  /// 取消当前验证并保存已获取的 Cookie（官方 2.2.9 兼容）
  Future<void> cancelAndSave(String pluginName) async {
    try {
      _imageFoundSub?.cancel();
      _disappearedSub?.cancel();
      if (_controller != null) {
        final cookieStr = await _controller!.getCookieString('https://placeholder');
        if (cookieStr.isNotEmpty) {
          await PluginCookieManager.instance
              .saveFromWebView(pluginName, 'https://placeholder', cookieStr);
        }
        _controller!.dispose();
        _controller = null;
      }
