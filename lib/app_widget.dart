...
class _AppWidgetState extends State<AppWidget>
      ...
      case 'download':
        ...
        Future.delayed(const Duration(milliseconds: 300), () {
          try {
            ...
            final path =
                navigationContext.routeState(listen: false).uri.path;
            if (path.contains('/download')) return;
            navigationContext.pushNamed('/settings/download/');
          } catch (e) {
            KazumiLogger()
                .w('Tray: failed to navigate to download page', error: e);
          }
        });
        break;
      case 'settings':
        ...
        Future.delayed(const Duration(milliseconds: 300), () {
          try {
            final navigationContext = rootNavigatorKey.currentContext;
            if (navigationContext == null || !navigationContext.mounted) return;
            navigationContext.pushNamed('/settings');
          } catch (e) {
            KazumiLogger()
                .w('Tray: failed to navigate to settings', error: e);
          }
        });
        break;
      case 'exit':
  ...
  /// 🆕 窗口移动/缩放时保存几何信息（防抖 800ms）
  DateTime _lastGeometrySave = DateTime.fromMillisecondsSinceEpoch(0);
  ...
  void _scheduleGeometrySave() {
    ...
    if (now.difference(_lastGeometrySave) < const Duration(milliseconds: 800)) {
      return;
    }
    _lastGeometrySave = now;
    unawaited(WindowStateService.save());
  }
  ...
  Future<void> _handleTray() async {
    // 🆕 尊重托盘开关（默认开启）
    if (!GStorage.getSetting(SettingsKeys.desktopTrayEnabled)) {
      return;
    }
    ...
    Menu trayMenu = Menu(items: [
      ...
      MenuItem.separator(),
      MenuItem(key: 'download', label: '打开下载页'),
