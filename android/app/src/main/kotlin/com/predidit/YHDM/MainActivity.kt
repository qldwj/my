...
class MainActivity: AudioServiceActivity() {
    ...
    override fun configureFlutterEngine(@NonNull flutterEngine: FlutterEngine) {
        ...
        shortcutChannel?.setMethodCallHandler { call, result ->
            if (call.method == "pinShortcut") {
                ...
                val ep = call.argument<Int>("ep") ?: 0
                val adapterName = call.argument<String>("adapterName") ?: ""
                if (id.isEmpty()) {
                    result.error("INVALID_ARGUMENT", "id required", null)
                } else {
                    val pinned = pinShortcut(id, name, ep, adapterName)
    ...
    /// 处理桌面快捷方式点击：把番剧参数传给 Flutter
    private fun handleShortcutIntent(intent: Intent?) {
        val i = intent ?: return
        if (i.action != SHORTCUT_ACTION) return
        val id = i.getStringExtra("kazumi_id") ?: ""
        val name = i.getStringExtra("kazumi_name") ?: ""
        val ep = i.getIntExtra("kazumi_ep", 0)
        val adapter = i.getStringExtra("kazumi_adapter") ?: ""
        ...
        shortcutChannel?.invokeMethod(
            ...
            mapOf(
                ...
                "name" to name,
                "ep" to ep,
                "adapterName" to adapter
            )
        )
    }

    /// 创建桌面快捷方式（Android 8+，需用户确认固定）
    private fun pinShortcut(id: String, name: String, ep: Int, adapterName: String): Boolean {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
            return false
        }
        val intent = Intent(this, MainActivity::class.java).apply {
            action = SHORTCUT_ACTION
            putExtra("kazumi_id", id)
            putExtra("kazumi_name", name)
            putExtra("kazumi_ep", ep)
            putExtra("kazumi_adapter", adapterName)
            addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP)
        }
        val shortcut = ShortcutInfoCompat.Builder(this, "kazumi_$id").apply {
